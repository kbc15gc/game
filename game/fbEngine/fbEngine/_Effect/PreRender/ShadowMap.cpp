...
void ShadowMap::Render()
{
	...
	FOR(count, SHADOWMAP_NUM)
	{
		...
		(*graphicsDevice()).Clear(0, nullptr, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_RGBA(1, 1, 1, 1), 1.0f, 0);
...
void ShadowMap::CalcLVPMatrixFromCamera()
{
	...
	for (UINT i = 0; i < SHADOWMAP_NUM; i++)
	{
		...
		//ライトビュー行列完成.
		D3DXMatrixInverse(&lightViewMatrix, nullptr, &lightViewMatrix);

		//ライトビュー行列を設定.
		_LVMatrix[i] = lightViewMatrix;
		
		//カメラの画角.
		float viewAngle = _Camera->GetViewAngle();

		float h = tan(viewAngle * 0.5f) * (shadowAreaTbl[i] * (i + 1)) * 2.0f;
